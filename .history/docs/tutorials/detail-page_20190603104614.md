...
```js
actionBuilder({
    node: "movieCharacteristics", // Noeud ciblé
    service: movieServices.loadMovieCharacteristics, // Service à appeler
    shouldDumpStoreOnActionCall: true, // Si on vide le noeud pendant le chargement
    status: "loaded" // Le status du noeud à la fin de l'appel
});
```
