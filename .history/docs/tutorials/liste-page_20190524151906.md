...
const movielistActions = listActionBuilder({
    service: movieServices.searchMovies,
    identifier: "movieList",
    getListOptions: () => movieListStore.getValue()
});

// On exporte l'action de load de la liste
export const loadMovieList = movielistActions.load;
// On exporte une fonction pour mettre à jour les élements du store.
export const updateMovieListProperties = movielistActions.updateProperties;
```
