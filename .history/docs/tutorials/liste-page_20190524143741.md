...
```
{
    // Le critère éventuel de recherche qui doit être un objet structuré
    criteria,
    // Par quel élement est ce la liste est triée
    sortBy,
    // Le caractère ascendant ou non pour le tri
    sortAsc,
    // Les élements de la liste (une partie au moins s'il y a pagination
    dataList,
    // Le nombre total d'éléments de la liste (utile pour réaliser la pagination
    totalCount;
}
```
