...
1. Modifie le `status` du noeud avec la valeur de `preLoading` (par défaut: `loading`) et passe `isLoading` à vrai
...
5. Dispatch le resultat du serveur dans le store, met le `status` du noeud à `status` et `isLoading` à faux
...
On instancie le bloc dans la page principale :
