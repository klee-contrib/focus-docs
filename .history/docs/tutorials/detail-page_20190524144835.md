...
+++    stores: [{ store: movieStore, properties: ['movieCharacteristics'] }], // Abonnement au store
...
Le synopsis est un attribut de notre entité, on bénéficie donc déjà de l'information dans le noeud `movieCharacteristics` du `movieStore`.
...
export const MovieSynopsis = React.createClass({
    ...
    stores: [{ store: movieStore, properties: ["movieCharacteristics"] }],
